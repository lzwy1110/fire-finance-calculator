  ...
  return (
    <div className="fixed inset-0 z-[130] flex items-center justify-center p-2 sm:p-4 bg-black/85 backdrop-blur-md animate-fadeIn overflow-x-hidden overflow-y-auto">
      <div className="bg-[#121216] border border-white/10 rounded-3xl max-w-3xl w-full shadow-2xl overflow-hidden max-h-[92vh] flex flex-col relative transform transition-all scale-100 min-w-0">
        ...
        <div className="p-3.5 sm:p-6 border-b border-white/10 flex items-start justify-between relative z-10 flex-shrink-0 w-full">
          <div className="space-y-1 min-w-0 flex-1 pr-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="px-2 py-0.5 rounded-lg bg-white/10 text-xs font-mono font-bold text-gray-300 shrink-0">
                {stock.symbol}
              </span>
              {/* US Market 15-min Delay Notice Badge */}
              {isUS && (
                <span className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-amber-500/10 border border-amber-500/20 text-[10px] sm:text-[11px] font-semibold text-amber-400 shrink-0">
            ...
            <div className="flex items-baseline gap-2.5 pt-0.5 flex-wrap">
              <span className="text-xl sm:text-2xl md:text-3xl font-black text-white font-mono tracking-tight">
                {currencySymbol} {priceDisplay}
              </span>

              {/* Dynamic Header Return: Daily in K-Line vs Period Return in Line */}
              <span
                className={`flex items-center text-xs sm:text-sm font-bold ${
                  headerReturn.isPositive ? 'text-emerald-400' : 'text-rose-400'
                }`}
              >
                {headerReturn.isPositive ? <ArrowUpRight className="w-3.5 h-3.5" /> : <ArrowDownRight className="w-3.5 h-3.5" />}
        ...
        <div className="overflow-y-auto overflow-x-hidden flex-1 flex flex-col w-full">
          {/* 2. Control Toolbar */}
          <div className="px-3 sm:px-6 py-2 bg-white/[0.02] border-b border-white/5 flex items-center justify-between gap-2 flex-wrap relative z-10 text-xs flex-shrink-0 w-full">
            {/* Smart Dynamic Timeframe Switcher */}
            <div className="flex items-center gap-0.5 sm:gap-1 bg-black/40 p-0.5 sm:p-1 rounded-xl border border-white/5">
              {chartType === 'line' ? (
                ...
                (['1d', '5d', '1m', '1y', '5y'] as LineRange[]).map((t) => (
                  <button
                    ...
                    className={`px-1.5 sm:px-2.5 py-1 rounded-lg font-bold uppercase text-[11px] sm:text-xs transition-all cursor-pointer ${
                      lineRange === t
                ...
                ([
                  ...
                  <button
                    ...
                    className={`px-1.5 sm:px-2.5 py-1 rounded-lg font-bold text-[11px] sm:text-xs transition-all cursor-pointer ${
            ...
            <div className="flex items-center gap-1 sm:gap-1.5">
              ...
              <div className="flex items-center gap-0.5 sm:gap-1 bg-black/40 p-0.5 sm:p-1 rounded-xl border border-white/5">
                <button
                  ...
                  className={`px-1.5 sm:px-2.5 py-1 rounded-lg font-bold text-[11px] sm:text-xs flex items-center gap-1 transition-all cursor-pointer ${
                    chartType === 'line'
